...
function ColorPallet() {
    ...
    return (
        <>
            <main className={`relative flex items-center justify-center h-screen flex-col ${darkMode ? 'dark:bg-slate-900' : 'dark:bg-slate-50'}`}>
                <div style={{
                    top: (copy ? '0px' : '-65px'),
                    transition: 'top 0.3s ease-in-out'
                }} className={`absolute z-10 w-full font-semibold text-center text-lg py-4 ${darkMode ? 'dark:bg-slate-50 dark:text-slate-800' : 'dark:bg-slate-800 dark:text-slate-50'}`}>
                    Color copiado!
                </div>
